...
Функция НазначениеСвойствИКатегорийОбъектов(ИмяПредопределенногоНабора)  ЭКСПОРТ
	...
	КонецЕсли;
	//Если ИмяПредопределенногоНабора = "" Тогда
	//	Возврат "Справочник_КонтактныеЛица";
	//КонецЕсли;
	Если ИмяПредопределенногоНабора = "Справочник_Контрагенты" Тогда
		Возврат "Справочник_Контрагенты";
	КонецЕсли;
	//Если ИмяПредопределенногоНабора = "" Тогда
	//	Возврат "Справочник_ЛичныеКонтакты";
	//КонецЕсли;
...
Функция ОпределениеЮрФизЛица(ЮрФизЛицо) ЭКСПОРТ
	...
	КонецЕсли;
	Если ЮрФизЛицо = "ЮрЛицоНеРезидент" Тогда
		Возврат Перечисления.ЮрФизЛицо.ЮрЛицо;
	КонецЕсли;
...
Процедура ЗаписьДополнительныхРеквизитов(Ссылка, Данные) ЭКСПОРТ
	//устанавливаем идентичность контрагента из реквизита Партнер
		
	
	Для Каждого ЭлементДопРеквизитов Из Данные.ДополнительныеРеквизиты Цикл
		
	КонецЦикла;
	////для контрагентов больше реквизитов нет. потренируемся на номенклатуре
КонецПроцедуры  

Функция ВидКИПоНаименованиюИТипуКИИВидуОбъекта(Наименование, ВидОбъектаКИ, ТипКИ) ЭКСПОРТ
	...
	Если Наименование = "Фактический адрес" И ВидОбъектаКИ = Перечисления.ВидыОбъектовКонтактнойИнформации.Контрагенты И ТипКИ = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		Возврат Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента;
	КонецЕсли;
	Если Наименование = "Юридический адрес" И ВидОбъектаКИ = Перечисления.ВидыОбъектовКонтактнойИнформации.Контрагенты И ТипКИ = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
	...
	Запрос.УстановитьПараметр("ВидОбъектаКонтактнойИнформации", ВидОбъектаКИ);
...
Функция ОпределениеТипаКонтактнойИнформации(ТипКонтактнойИнформации) ЭКСПОРТ
	Если ТипКонтактнойИнформации = "Адрес" Тогда
